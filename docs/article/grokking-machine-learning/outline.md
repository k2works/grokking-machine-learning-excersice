...
| :--- | :--- | :--- | :--- |
| 01 | 用語と全体像、Python で扱うデータ表現 | JVM 上で ML を書く意味 | 型で概念を表す |
| 02 | 学習の種類とデータ形状 | sealed class での分類 | 判別共用体での分類 |
| 03 | dataclass と純関数でトリックを表現 | data class と不変更新 | レコード型とパイプライン |
| 04 | 訓練／テスト分割、L1・L2 正則化 | 高階関数で正則化項を差し替え | 関数合成でモデルを組み替え |
| 05 | パーセプトロンの更新則 | 分類器インターフェース | 部分適用で更新則を注入 |
| 06 | シグモイドと対数損失 | 演算子の拡張関数 | 演算子定義と単位付き量 |
| 07 | 混同行列と指標の実装 | enum と when で指標を網羅 | 判別共用体で網羅性検査 |
| 08 | 辞書によるカウント、ラプラス平滑化 | Map と groupingBy | Map と畳み込み |
| 09 | 再帰的な木構築 | sealed class の木 | 判別共用体の木とパターンマッチ |
| 10 | 逆伝播の手計算実装 | 配列演算と拡張関数 | 再帰関数と不変配列 |
| 11 | マージン最大化とカーネル | 関数型インターフェースでカーネル注入 | 関数値としてのカーネル |
| 12 | バギング・AdaBoost | シーケンスと遅延評価 | Seq と関数合成 |
| 13 | データ前処理からモデル評価まで | 型安全なパイプライン | 全体を関数合成で組む |
